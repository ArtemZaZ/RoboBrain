...
class ControllerMotor(ControllerBase):
    ...
    def __init__(self, owner=None, addr=0):
        ...
        self.MotorParam = ((None, None, None, None, None, None, None),
                           (None, None, None, None, None, None,
                            None))  # Кортеж кортежей, содержащих информацию о двух моторах. Значение скорости, значение одометра, п, и, д коэффициенты, результирующий ШИМ, интегр сумма
        ...
        self.__CommandList = self._ControllerBase__CommandList.update({
            # Команды управления
            # name, type, length
            0xCC: ['work mode', DT_UINT8],
        # пример. Переделать в списки. Название - тип. Параметр задавать не текстом, а номером.
    ...
    def SetWorkMode(self,
                    workMode=0):  # Инициализация режима работы (2 - ШИМ, 1 - ПИД, 0 - пока используется как выключение ручного режима)
    ...
    def BasicOnGetParamNew(self, prmNumber, prm):
        ...
        super().BasicOnGetParam(prmNumber,
                                prm)  # Функция вызываемая при приеме параметра, описанная в модуле ControllerBase
